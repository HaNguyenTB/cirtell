...
\begin{enumerate}[leftmargin=1.2cm]
    \item Người dùng đăng nhập qua Azure AD (MSAL) \cite{msalDoc}; nhận \emph{ID token} từ Microsoft.
    \item Backend xác thực ID token này, sau đó cấp phát JWT nội bộ chứa \texttt{user\_id}, \texttt{tenant\_id}, \texttt{roles}.
    \item Mọi request tiếp theo đính kèm JWT trong header \texttt{Authorization: Bearer ...}.
